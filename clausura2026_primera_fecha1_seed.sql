-- =============================================================================
-- FECHA 1 - PRIMERA DIVISIÓN (Div. Superior)
-- Torneo: Clausura Joaquín "Cacho" Méndez
-- Sábado 1 de Agosto de 2026 - Horario: 15:30 hs
-- Zona 1 (9 equipos): ALM.FLORIDA, RIVER PLATE, ARG.DEL SUD, INDEPENDIENTE,
--                     ALVARADO, LIBERTAD, QUILMES, AL VER VERAS, TALLERES
...
BEGIN
  ...
  SELECT id INTO v_div   FROM public.divisions
    WHERE name = 'Primera División' LIMIT 1;
  ...
  -- ══ FECHA 1 - ZONA 1 - SÁBADO 01/08/2026 15:30 ════════════════════════════
  -- Alm.Florida vs River Plate
  INSERT INTO public.matches
    (tournament_id, division_id, zone_id, round_number, home_team_id, away_team_id, status, match_date)
  SELECT v_tourn, v_div, v_z1, 1, t_almagro, t_river, 'scheduled',
         '2026-08-01T15:30:00-03:00'::timestamptz
  WHERE NOT EXISTS (
    SELECT 1 FROM public.matches
    WHERE tournament_id = v_tourn AND division_id = v_div AND zone_id = v_z1
      AND round_number = 1 AND home_team_id = t_almagro AND away_team_id = t_river
  );

  -- Arg del Sud vs Independiente
  INSERT INTO public.matches
    (tournament_id, division_id, zone_id, round_number, home_team_id, away_team_id, status, match_date)
  SELECT v_tourn, v_div, v_z1, 1, t_argsud, t_indep, 'scheduled',
         '2026-08-01T15:30:00-03:00'::timestamptz
  WHERE NOT EXISTS (
    SELECT 1 FROM public.matches
    WHERE tournament_id = v_tourn AND division_id = v_div AND zone_id = v_z1
      AND round_number = 1 AND home_team_id = t_argsud AND away_team_id = t_indep
  );

  -- Alvarado vs Libertad
  INSERT INTO public.matches
    (tournament_id, division_id, zone_id, round_number, home_team_id, away_team_id, status, match_date)
  SELECT v_tourn, v_div, v_z1, 1, t_alvarado, t_libertad, 'scheduled',
         '2026-08-01T15:30:00-03:00'::timestamptz
  WHERE NOT EXISTS (
    SELECT 1 FROM public.matches
    WHERE tournament_id = v_tourn AND division_id = v_div AND zone_id = v_z1
      AND round_number = 1 AND home_team_id = t_alvarado AND away_team_id = t_libertad
  );

  -- Quilmes vs Al Ver Veras
  INSERT INTO public.matches
    (tournament_id, division_id, zone_id, round_number, home_team_id, away_team_id, status, match_date)
  SELECT v_tourn, v_div, v_z1, 1, t_quilmes, t_alver, 'scheduled',
         '2026-08-01T15:30:00-03:00'::timestamptz
  WHERE NOT EXISTS (
    SELECT 1 FROM public.matches
    WHERE tournament_id = v_tourn AND division_id = v_div AND zone_id = v_z1
      AND round_number = 1 AND home_team_id = t_quilmes AND away_team_id = t_alver
  );
  -- Nota: TALLERES tiene LIBRE en Fecha 1 (zona 1 - Primera, 9 equipos)
  -- No se inserta partido para Talleres en esta fecha.

  -- ══ FECHA 1 - ZONA 2 - SÁBADO 01/08/2026 15:30 ════════════════════════════
  -- San José vs Círculo Deportivo
  INSERT INTO public.matches
    (tournament_id, division_id, zone_id, round_number, home_team_id, away_team_id, status, match_date)
  SELECT v_tourn, v_div, v_z2, 1, t_sanjose, t_circulo, 'scheduled',
         '2026-08-01T15:30:00-03:00'::timestamptz
  WHERE NOT EXISTS (
    SELECT 1 FROM public.matches
    WHERE tournament_id = v_tourn AND division_id = v_div AND zone_id = v_z2
      AND round_number = 1 AND home_team_id = t_sanjose AND away_team_id = t_circulo
  );

  -- Gral. Mitre vs Boca Juniors
  INSERT INTO public.matches
    (tournament_id, division_id, zone_id, round_number, home_team_id, away_team_id, status, match_date)
  SELECT v_tourn, v_div, v_z2, 1, t_mitre, t_boca, 'scheduled',
         '2026-08-01T15:30:00-03:00'::timestamptz
  WHERE NOT EXISTS (
    SELECT 1 FROM public.matches
    WHERE tournament_id = v_tourn AND division_id = v_div AND zone_id = v_z2
      AND round_number = 1 AND home_team_id = t_mitre AND away_team_id = t_boca
  );

  -- Unión vs Racing
  INSERT INTO public.matches
    (tournament_id, division_id, zone_id, round_number, home_team_id, away_team_id, status, match_date)
  SELECT v_tourn, v_div, v_z2, 1, t_union, t_racing, 'scheduled',
         '2026-08-01T15:30:00-03:00'::timestamptz
  WHERE NOT EXISTS (
    SELECT 1 FROM public.matches
    WHERE tournament_id = v_tourn AND division_id = v_div AND zone_id = v_z2
      AND round_number = 1 AND home_team_id = t_union AND away_team_id = t_racing
  );

  -- San Isidro vs Cadetes (SM)
  INSERT INTO public.matches
    (tournament_id, division_id, zone_id, round_number, home_team_id, away_team_id, status, match_date)
  SELECT v_tourn, v_div, v_z2, 1, t_sanisidro, t_cadetes, 'scheduled',
         '2026-08-01T15:30:00-03:00'::timestamptz
  WHERE NOT EXISTS (
    SELECT 1 FROM public.matches
    WHERE tournament_id = v_tourn AND division_id = v_div AND zone_id = v_z2
      AND round_number = 1 AND home_team_id = t_sanisidro AND away_team_id = t_cadetes
  );

  -- Chapadmalal vs Kimberley
  INSERT INTO public.matches
    (tournament_id, division_id, zone_id, round_number, home_team_id, away_team_id, status, match_date)
  SELECT v_tourn, v_div, v_z2, 1, t_chap, t_kimber, 'scheduled',
         '2026-08-01T15:30:00-03:00'::timestamptz
  WHERE NOT EXISTS (
    SELECT 1 FROM public.matches
    WHERE tournament_id = v_tourn AND division_id = v_div AND zone_id = v_z2
      AND round_number = 1 AND home_team_id = t_chap AND away_team_id = t_kimber
  );

  -- ══ FECHA 1 - ZONA 3 - SÁBADO 01/08/2026 15:30 ════════════════════════════
  -- San Lorenzo vs Deportivo Norte
  INSERT INTO public.matches
    (tournament_id, division_id, zone_id, round_number, home_team_id, away_team_id, status, match_date)
  SELECT v_tourn, v_div, v_z3, 1, t_sanlorenzo, t_depnorte, 'scheduled',
         '2026-08-01T15:30:00-03:00'::timestamptz
  WHERE NOT EXISTS (
    SELECT 1 FROM public.matches
    WHERE tournament_id = v_tourn AND division_id = v_div AND zone_id = v_z3
      AND round_number = 1 AND home_team_id = t_sanlorenzo AND away_team_id = t_depnorte
  );

  -- Gral. Urquiza vs Mar del Plata
  INSERT INTO public.matches
    (tournament_id, division_id, zone_id, round_number, home_team_id, away_team_id, status, match_date)
  SELECT v_tourn, v_div, v_z3, 1, t_urquiza, t_mdp, 'scheduled',
         '2026-08-01T15:30:00-03:00'::timestamptz
  WHERE NOT EXISTS (
    SELECT 1 FROM public.matches
    WHERE tournament_id = v_tourn AND division_id = v_div AND zone_id = v_z3
      AND round_number = 1 AND home_team_id = t_urquiza AND away_team_id = t_mdp
  );

  -- El Cañón vs Colegiales
  INSERT INTO public.matches
    (tournament_id, division_id, zone_id, round_number, home_team_id, away_team_id, status, match_date)
  SELECT v_tourn, v_div, v_z3, 1, t_canon, t_colegia, 'scheduled',
         '2026-08-01T15:30:00-03:00'::timestamptz
  WHERE NOT EXISTS (
    SELECT 1 FROM public.matches
    WHERE tournament_id = v_tourn AND division_id = v_div AND zone_id = v_z3
      AND round_number = 1 AND home_team_id = t_canon AND away_team_id = t_colegia
  );

  -- Banfield vs Nación
  INSERT INTO public.matches
    (tournament_id, division_id, zone_id, round_number, home_team_id, away_team_id, status, match_date)
  SELECT v_tourn, v_div, v_z3, 1, t_banfield, t_nacion, 'scheduled',
         '2026-08-01T15:30:00-03:00'::timestamptz
  WHERE NOT EXISTS (
    SELECT 1 FROM public.matches
    WHERE tournament_id = v_tourn AND division_id = v_div AND zone_id = v_z3
      AND round_number = 1 AND home_team_id = t_banfield AND away_team_id = t_nacion
  );
  -- Nota: ONCE UNIDOS tiene LIBRE en Fecha 1 (zona 3 - 9 equipos)

END $$;

-- ─── VERIFICACIÓN ─────────────────────────────────────────────────────────────
-- SELECT count(*) FROM public.matches
--   JOIN public.divisions d ON d.id = division_id
--   JOIN public.zones z ON z.id = zone_id
--   WHERE d.name = 'Primera División' AND round_number = 1;
-- Esperado: 13 partidos (4 zona1 + 5 zona2 + 4 zona3)
